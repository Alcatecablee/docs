...
const stages: Stage[] = [
  {
    ...
    substages: ["Scanning Stack Overflow", "Analyzing GitHub discussions", "Mining Reddit threads"],
  },
  {
    id: 2,
    ...
    icon: LightBulbIcon,
    substages: ["Extracting pricing data", "Identifying pain points", "Analyzing sentiment"],
  },
  {
    id: 3,
    ...
    substages: ["Structuring content", "Generating PDF", "Applying branding"],
  },
  {
    id: 4,
    ...
    icon: ChartPieIcon,
    substages: ["Assessing coverage", "Scoring sources", "Final validation"],
  },
];

export default function BattlecardProgress() {
  ...
  const [battlecardId, setBattlecardId] = useState<string>("");
  ...
  const downloadActivityLog = useCallback(() => {
    ...
    a.href = url;
    a.download = `battlecard-log-${sessionId}.txt`;
    a.click();
  ...
  const connectToSSE = useCallback((competitor: string) => {
    ...
    addActivityLog(`🚀 Initiating competitive intelligence research for ${competitor}`, 'info');
    const eventSource = new EventSource(`/api/battlecard-progress/${sessionId}`);
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (progressData.battlecardId) {
          setBattlecardId(progressData.battlecardId);
        }

        if (progressData.pdfUrl) {
          setPdfUrl(progressData.pdfUrl);
        }
        ...
        if (progressData.complete || progressData.status === 'complete') {
          ...
          addActivityLog('🎉 Battlecard generation completed successfully! Your competitive intelligence is ready.', 'success');
        }
  ...
  useEffect(() => {
    ...
    competitor = navState.competitorName;

    if (!competitor) {
      toast({
        title: "Missing Competitor",
        description: "No competitor name provided for generation",
        variant: "destructive",
      });
      navigate("/battlecard");
      return;
  ...
  const handleGoBack = () => {
    if (isComplete || hasError) {
      ...
      const confirmLeave = window.confirm(
        "Battlecard generation is in progress. Are you sure you want to leave?"
      );
      if (confirmLeave) {
        navigate("/battlecard");
      }
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-[#0A0E12] via-[#0E1317] to-[#12171D] flex flex-col overflow-hidden relative">
      ...
      <header className="relative bg-gradient-to-r from-[#1A1F26]/90 to-[#22262E]/90 backdrop-blur-xl border-b border-[rgb(102,255,228)]/20 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between z-20 shadow-2xl">
        <div className="flex items-center gap-3 md:gap-4">
          <Button
            ...
            aria-label="Back to Battlecard Generator"
          >
            ...
            <div className="h-full overflow-y-auto bg-gradient-to-br from-[#0E1317]/50 to-[#12171D]/50 backdrop-blur-sm">
              <div className="p-4 md:p-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-white/60">Real-time competitive intelligence stream</p>
                </div>
                              ...
                              <p className={`text-xs leading-relaxed mt-0.5 ${
                                ...
                                {log.data && (
                                  <span className="text-white/50 ml-1 font-normal">
                                    ...
                                    {log.data.insightsExtracted && `• ${log.data.insightsExtracted} insights`}
                                    {log.data.pagesGenerated && `• ${log.data.pagesGenerated} pages`}
                                  </span>
                                )}
                              </p>
                            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Card className="glass-morphism border-[rgb(102,255,228)]/20 p-3 hover:border-[rgb(102,255,228)]/40 transition-all duration-300">
                    <div className="flex items-center gap-2">
                      <MagnifyingGlassIcon className="h-4 w-4 text-[rgb(102,255,228)]" />
                      <div>
                        <div className="text-xs text-white/60">Sources</div>
                        <div className="text-lg font-bold text-white">{metrics.sourcesAnalyzed}</div>
                      </div>
                    </div>
                  </Card>
                  <Card className="glass-morphism border-[rgb(102,255,228)]/20 p-3 hover:border-[rgb(102,255,228)]/40 transition-all duration-300">
                    <div className="flex items-center gap-2">
                      <LightBulbIcon className="h-4 w-4 text-[rgb(102,255,228)]" />
                      <div>
                        <div className="text-xs text-white/60">Insights</div>
                        <div className="text-lg font-bold text-white">{metrics.insightsExtracted}</div>
                      </div>
                    </div>
                  </Card>
                </div>
              ...
              <div className="p-4 md:p-6 lg:p-8 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm md:text-base text-white/70 leading-relaxed">
                    {isComplete
                      ? "Your competitive intelligence battlecard is ready for download"
                      : hasError
                      ? "We encountered an issue during the generation process"
                      : "Our AI is gathering comprehensive competitive intelligence from multiple sources"}
                  </p>
                </div>
                ...
                {isComplete && !hasError && (
                  <Card className="glass-morphism-success border-2 border-[rgb(102,255,228)]/50 shadow-2xl shadow-[rgb(102,255,228)]/20 p-5 animate-slideDown" role="status">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <h3 className="text-sm font-bold text-[rgb(102,255,228)] mb-2">Battlecard Ready!</h3>
                        <p className="text-sm text-white/80 leading-relaxed">Your comprehensive competitive intelligence battlecard is ready to download</p>
                      </div>
                    </div>
                    <div className="mt-4 flex gap-2 flex-wrap">
                      <Button 
                        onClick={handleViewBattlecard} 
                        size="sm" 
                        className="bg-gradient-to-r from-[rgb(102,255,228)] to-[rgb(80,200,180)] hover:from-white hover:to-[rgb(102,255,228)] text-[rgb(14,19,23)] transition-all duration-300 font-bold shadow-lg hover:shadow-[rgb(102,255,228)]/50 hover:scale-105"
                      >
                        Download Battlecard
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base font-bold text-white">Research Pipeline</h2>
                  </div>
                          ...
                          <div className="relative">
                            <div
                              className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${
                                isCompleted
                                  ...
                                  : isActive
                                  ? "bg-gradient-to-br from-[rgb(102,255,228)]/30 to-[rgb(80,200,180)]/30 border-2 border-[rgb(102,255,228)]"
                            ...
                            >
                              {isCompleted ? (
                                ...
                                <Icon className={`h-5 w-5 ${isActive ? "text-[rgb(102,255,228)]" : "text-white/40"}`} />
                              )}
                            </div>
                            {isActive && (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Card className="glass-morphism border-[rgb(102,255,228)]/20 p-4 hover:scale-105 transition-transform duration-300">
                        ...
                        <div className="text-xs text-white/40 mt-1">Reddit, SO, GitHub, YouTube...</div>
                      </Card>
                      <Card className="glass-morphism border-[rgb(102,255,228)]/20 p-4 hover:scale-105 transition-transform duration-300">
                        <div className="text-xs text-white/60 mb-1">Insights Extracted</div>
                        <div className="text-2xl font-black text-white">{metrics.insightsExtracted}</div>
                        <div className="text-xs text-white/40 mt-1">Key competitive insights</div>
                      </Card>
                    </div>
                  </div>
                )}
                ...
                <Card className="glass-morphism border-white/10 p-4">
                  ...
                  <ul className="space-y-2.5 text-xs text-white/70 leading-relaxed">
                    <li className="flex items-start gap-2">
                      ...
                      <span>We analyze <strong className="text-white">Reddit, Stack Overflow, GitHub, and YouTube</strong> for comprehensive competitive intelligence</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-[rgb(102,255,228)] mt-0.5 font-bold">•</span>
                      <span>Battlecards include <strong className="text-white">pricing intelligence, customer sentiment, and technical insights</strong></span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-[rgb(102,255,228)] mt-0.5 font-bold">•</span>
                      <span><strong className="text-white">Quality scoring and source attribution</strong> ensure actionable insights</span>
                    </li>
                  </ul>
                </Card>
              </div>
            ...
            <div className="h-full bg-[rgb(20,25,30)] flex flex-col">
              ...
              <div className="px-4 py-3 border-b border-white/10 bg-gradient-to-r from-[#1A1F26]/90 to-[#22262E]/90 backdrop-blur-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-white/60 font-mono truncate flex-1">{competitorName}</span>
                </div>
                <h2 className="text-sm font-bold bg-gradient-to-r from-white to-[rgb(102,255,228)] bg-clip-text text-transparent">
                  {pdfUrl ? "📄 Battlecard Preview" : "🎯 Target Competitor"}
                </h2>
              </div>
              
              {/* Preview Content */}
              <div className="flex-1 relative bg-gradient-to-br from-[#0E1317] to-[#12171D] overflow-hidden flex items-center justify-center">
                {pdfUrl ? (
                  <iframe
                    src={pdfUrl}
                    className="w-full h-full border-0"
                    title="Battlecard PDF Preview"
                  />
                ) : (
                  <div className="text-center space-y-4 p-8">
                    <DocumentTextIcon className="h-20 w-20 text-white/20 mx-auto animate-pulse" />
                    <div>
                      <p className="text-sm font-semibold text-white/60 mb-1">Battlecard Preview</p>
                      <p className="text-xs text-white/40">PDF will appear here when ready</p>
                    </div>
                  </div>
                )}
      ...
      {/* Custom Styles - EXACT SAME AS GENERATION PROGRESS */}
      <style>{`
        @keyframes slideIn {
          from {
            opacity: 0;
